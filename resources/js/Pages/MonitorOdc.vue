...
const destinos = {
    ...
    '0141': '0141 DEP. CHARCUTERÍA / FRESCOS',
    '0140': '0140 DEP. PERECEDEROS A',
    '0130': '0130 DEP. PERECEDEROS B',
    '0131': '0131 DEP. PERECEDEROS C',
    '0160': '0160 DEP. FRESCOS / LÁCTEOS',
    '0161': '0161 DEP. FRESCOS B',
    '0171': '0171 DEP. FRESCOS C'
};

// Mostrar sub-depósitos únicamente cuando la pestaña activa sea Perecederos
const destinosVisibles = computed(() => {
...
watch(pestanaActiva, (nuevaPestana) => {
    if (nuevaPestana !== 'perecederos' && !['todas', '0101', '0102'].includes(filtroSucursal.value)) {
...
const cargarOrdenes = async (showLoading = true) => {
    ...
    try {
        const resp = await axios.get('/api/monitor-odc/ordenes');
...
const ordenesAgrupadas = computed(() => {
    const grupos = {};
    const terminoBusqueda = String(busquedaFiltro.value || '').toLowerCase().trim();
    ...
    ordenes.value.forEach(o => {
        ...
        // Si hay una búsqueda por texto activa, se busca en todas las categorías sin importar la pestaña seleccionada
        if (terminoBusqueda === '') {
            if (pestanaActiva.value === 'secos' && Number(o.es_secos || 0) === 0) return;
            
            if (pestanaActiva.value === 'perecederos') {
                if (Number(o.es_perecederos || 0) === 0) return;
                // Sub-filtros Perecederos (Fase 5)
                if (subFiltroPerecederos.value === 'carnes' && Number(o.es_carnes || 0) === 0) return;
                if (subFiltroPerecederos.value === 'charcuteria' && Number(o.es_charcuteria || 0) === 0) return;
                if (subFiltroPerecederos.value === 'pescaderia' && Number(o.es_pescaderia || 0) === 0) return;
                if (subFiltroPerecederos.value === 'congelados' && Number(o.es_congelados || 0) === 0) return;
            }
            
            if (pestanaActiva.value === 'fruver') {
                if (Number(o.es_fruver || 0) === 0) return;
                // Sub-filtros Fruver (Fase 4)
                if (subFiltroFruver.value === 'frutas' && Number(o.es_fruta || 0) === 0) return;
                if (subFiltroFruver.value === 'verduras' && Number(o.es_verdura || 0) === 0) return;
                if (subFiltroFruver.value === 'hortalizas' && Number(o.es_hortaliza || 0) === 0) return;
            }
        }

        // Filtrar por sucursal
        const destCode = (o.destino && String(o.destino).trim() !== '' && String(o.destino) !== 'undefined') ? String(o.destino) : '0101';
        if (filtroSucursal.value !== 'todas') {
            const esPerecedero = Number(o.es_perecederos || 0) === 1;
            if (esPerecedero) {
                // Únicamente para Perecederos: Si la sucursal seleccionada es 0101 (Hiper Suraki), incluir depósitos 01% (0141, 0140, 0130, etc.)
                const matchSuraki = (filtroSucursal.value === '0101' || filtroSucursal.value === '01') && destCode.startsWith('01');
                const matchExact = destCode === filtroSucursal.value;
                if (!matchSuraki && !matchExact) return;
            } else {
                // Para Secos y Fruver: mantener filtrado estricto por destino sin modificar nada
                if (destCode !== filtroSucursal.value) return;
            }
        }

        // Filtrar por Estado (Pendiente vs Habilitadas vs Agendadas/Completadas)
        ...
        if (filtroEstado.value === 'habilitadas' && o.estatus_habilitacion !== 'habilitada') return;
        ...
        if (filtroFechaInicio.value || filtroFechaFin.value) {
            ...
            try {
                if (o.fecha_emision) {
                    fechaOC = new Date(o.fecha_emision).toISOString().split('T')[0];
                }
            } catch (e) {}
            if (filtroFechaInicio.value && fechaOC && fechaOC < filtroFechaInicio.value) return;
            if (filtroFechaFin.value && fechaOC && fechaOC > filtroFechaFin.value) return;
        }

        // Lógica de filtrado de búsqueda
        const numOcStr = String(o.numero_oc || o.Numero_OC || o.c_DOCUMENTO || '');
        const provStr = String(o.proveedor || o.Nombre_Proveedor || o.c_DESCRIPCION || '');
        const coincide = terminoBusqueda === '' || 
                         numOcStr.toLowerCase().includes(terminoBusqueda) || 
                         provStr.toLowerCase().includes(terminoBusqueda);

        if (coincide) {
            if (!grupos[destCode]) {
                grupos[destCode] = {
                    codigo: destCode,
                    nombre: destinos[destCode] || `Destino ${destCode}`,
                    ordenes: []
                };
            }
            grupos[destCode].ordenes.push(o);
        }
    });
    // Convertir a array y ordenar por código de destino
    return Object.values(grupos).sort((a, b) => String(a.codigo || '').localeCompare(String(b.codigo || '')));
});
...
const deshabilitarOrden = async (numero_oc) => {
    if (!confirm(`¿Estás seguro de que deseas restablecer la orden ${numero_oc} a estado pendiente? Esto eliminará su habilitación actual.`)) return;
    
    try {
        await axios.post(`/api/odc/${numero_oc}/deshabilitar`);
        // Recargar órdenes
        cargarOrdenes(false);
    } catch (e) {
        alert(e.response?.data?.error || 'Error al restablecer la orden.');
        console.error(e);
    }
};
                ...
                <div class="flex flex-wrap items-center gap-3 w-full md:w-auto">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <select v-model="filtroSucursal" class="block w-full sm:w-auto py-2 px-3 border border-slate-300 rounded-xl leading-5 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 shadow-sm">
                            <option value="todas">Hipersuraki</option>
                            <option v-for="(nombre, codigo) in destinosVisibles" :key="codigo" :value="codigo">
                                {{ nombre }}
                            </option>
                        </select>
                    </div>
        ...
        <div class="py-10 bg-slate-50 min-h-screen">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
                ...
                <div class="flex flex-wrap gap-2 mb-6">
                    <button @click="pestanaActiva = 'total'" 
                            :class="pestanaActiva === 'total' ? 'bg-slate-800 text-white shadow-md' : 'bg-white text-slate-600 border border-slate-200 hover:bg-slate-50'"
                ...
                <div class="flex flex-wrap gap-2 mb-6">
                    ...
                    <button v-if="userRole === 'admin'"
                            @click="filtroEstado = 'habilitadas'"
                            :class="filtroEstado === 'habilitadas' ? 'bg-blue-600 text-white shadow-md' : 'bg-white text-blue-600 border border-blue-200 hover:bg-blue-50'"
                            class="px-4 py-2 rounded-xl font-bold text-xs transition-all flex items-center gap-1.5"
                            title="Órdenes habilitadas para agendar cita (Vista de Admin)">
                        ⚡ Habilitadas
                    </button>
                            ...
                            <table class="w-full text-left text-sm whitespace-nowrap">
                                <thead class="bg-slate-50 text-slate-500 font-bold uppercase text-[10px] tracking-wider">
                                    <tr>
                                        ...
                                        <th v-if="pestanaActiva === 'total'" class="px-6 py-4 text-center">Bultos / KG</th>
                                ...
                                <tbody class="divide-y divide-slate-100">
                                    <template v-for="odc in grupo.ordenes" :key="odc.numero_oc">
                                        ...
                                        <td class="px-6 py-4">
                                            <div class="space-y-1">
                                                <div>
                                                    <span v-if="odc.estatus_habilitacion === 'agendada'" class="bg-emerald-100 text-emerald-700 font-black px-2 py-0.5 rounded text-[10px] uppercase">Agendada</span>
                                                    <span v-else-if="odc.estatus_habilitacion === 'habilitada'" class="bg-blue-100 text-blue-700 font-black px-2 py-0.5 rounded text-[10px] uppercase">Habilitada</span>
                                                    <span v-else class="bg-slate-100 text-slate-600 font-black px-2 py-0.5 rounded text-[10px] uppercase">Pendiente</span>
                                                </div>
                                                <div v-if="odc.fecha_envio_comprador || odc.fecha_emision || odc.fecha_odc" class="text-[10px] text-indigo-700 font-semibold flex items-center gap-1" title="Fecha y hora en que el comprador envió/habilitó la orden">
                                                    <span>📤</span> {{ formatFechaHora(odc.fecha_envio_comprador || odc.fecha_emision || odc.fecha_odc) }}
                                                </div>
                                                <div v-if="odc.fecha_registro_cita" class="text-[10px] text-purple-700 font-semibold flex items-center gap-1" title="Fecha y hora en que el proveedor registró la cita">
                                                    <span>⏱️</span> {{ formatFechaHora(odc.fecha_registro_cita) }}
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td v-if="pestanaActiva === 'total'" class="px-6 py-4 text-center">
                                            <div class="flex flex-col items-center gap-1">
                                                <span v-if="Number(odc.bultos) > 0" class="font-bold text-slate-700 bg-slate-100 px-2 py-0.5 rounded-md text-[10px]">{{ odc.bultos }} bultos</span>
                                                <span v-if="Number(odc.total_kg_carnes) > 0 || Number(odc.total_kg_charcuteria) > 0 || Number(odc.total_kg_pescaderia) > 0 || Number(odc.total_kg_congelados) > 0 || Number(odc.total_kg_frutas) > 0 || Number(odc.total_kg_verduras) > 0 || Number(odc.total_kg_hortalizas) > 0" class="font-bold text-slate-700 bg-slate-100 px-2 py-0.5 rounded-md text-[10px]">
                                                    {{ 
                                                        (Number(odc.total_kg_carnes || 0) + Number(odc.total_kg_charcuteria || 0) + Number(odc.total_kg_pescaderia || 0) + Number(odc.total_kg_congelados || 0) + Number(odc.total_kg_frutas || 0) + Number(odc.total_kg_verduras || 0) + Number(odc.total_kg_hortalizas || 0)).toFixed(2) 
                                                    }} kg
                                                </span>
                                                <span v-if="Number(odc.total_und_carnes) > 0 || Number(odc.total_und_charcuteria) > 0 || Number(odc.total_und_pescaderia) > 0 || Number(odc.total_und_congelados) > 0 || Number(odc.total_und_frutas) > 0 || Number(odc.total_und_verduras) > 0 || Number(odc.total_und_hortalizas) > 0" class="font-bold text-slate-700 bg-slate-100 px-2 py-0.5 rounded-md text-[10px]">
                                                    {{ 
                                                        Math.round(Number(odc.total_und_carnes || 0) + Number(odc.total_und_charcuteria || 0) + Number(odc.total_und_pescaderia || 0) + Number(odc.total_und_congelados || 0) + Number(odc.total_und_frutas || 0) + Number(odc.total_und_verduras || 0) + Number(odc.total_und_hortalizas || 0))
                                                    }} und
                                                </span>
                                            </div>
                                        </td>
                                        ...
                                        <td class="px-8 py-4 text-right">
                                            <div class="flex items-center justify-end gap-2">
                                                <Link v-if="$page.props.auth.user.role === 'comprador' || $page.props.auth.user.role === 'admin'"
                                                    ...
                                                    class="text-blue-600 hover:text-blue-800 font-bold bg-blue-50 px-2.5 py-1 rounded-lg transition-colors inline-flex items-center gap-1 opacity-0 group-hover:opacity-100 whitespace-nowrap"
                                                ...
                                                <button v-if="$page.props.auth.user.role === 'admin' && odc.estatus_habilitacion === 'habilitada'"
                                                    @click.stop="deshabilitarOrden(odc.numero_oc)"
                                                    class="text-red-600 hover:text-red-800 bg-red-50 p-1.5 rounded-lg transition-colors flex-shrink-0 opacity-0 group-hover:opacity-100"
                                                    title="Restablecer (Deshabilitar y borrar cuenta de prueba)">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path></svg>
                                                </button>
                                            </div>
                                        </td>
                                    ...
                                    <tr v-if="filasExpandidas.includes(odc.numero_oc)" class="bg-slate-50 border-b-2 border-slate-200">
                                        <td colspan="9" class="px-8 py-6">
                                            <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                                                ...
                                                <div v-if="odc.fecha_envio_comprador || odc.fecha_emision || odc.fecha_odc || odc.fecha_registro_cita || odc.fecha_completada">
                                                    <h4 class="text-xs font-black text-slate-400 uppercase tracking-widest mb-3">Trazabilidad de Tiempos</h4>
                                                    <div class="space-y-2">
                                                        <div v-if="odc.fecha_envio_comprador || odc.fecha_emision || odc.fecha_odc" class="flex justify-between items-center text-xs">
                                                            <span class="text-indigo-600 font-bold">📤 Envío Comprador</span>
                                                            <span class="font-bold text-indigo-900">{{ formatFechaHora(odc.fecha_envio_comprador || odc.fecha_emision || odc.fecha_odc) }}</span>
                                                        </div>
                                                        <div v-if="odc.fecha_registro_cita" class="flex justify-between items-center text-xs">
                                                            <span class="text-purple-600 font-bold">⏱️ Registro Cita</span>
                                                            <span class="font-bold text-purple-900">{{ formatFechaHora(odc.fecha_registro_cita) }}</span>
                                                        </div>
                                                        <div v-if="odc.fecha_completada" class="flex justify-between items-center text-xs">
                                                            <span class="text-emerald-600 font-bold">✅ Completada en Muelle</span>
                                                            <span class="font-bold text-emerald-950">
                                                                {{ formatFechaHora(odc.fecha_completada) }}
                                                                <span v-if="odc.completada_por_nombre" class="font-normal text-emerald-600">· por {{ odc.completada_por_nombre }}</span>
                                                            </span>
                                                        </div>
                                                        <div v-else class="flex justify-between items-center text-xs">
                                                            <span class="text-amber-600 font-bold">⏳ Estado Recepción</span>
                                                            <span class="font-medium text-amber-800">Pendiente por recibir</span>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
