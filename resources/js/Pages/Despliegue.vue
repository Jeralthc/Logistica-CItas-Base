        ...
        <template #header>
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-2 md:gap-4">
                <h2 class="font-bold text-xl sm:text-2xl text-slate-800 leading-tight">
                    Centro de Control de Cambios <span class="text-red-600 font-normal block sm:inline">| Despliegue y Recuperación</span>
                </h2>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div class="bg-white p-6 rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100 lg:col-span-2">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="space-y-3">
                                    <div class="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-100 pb-2 gap-1 sm:gap-2">
                                        <span class="text-xs sm:text-sm text-slate-500 font-medium">Versión Laravel:</span>
                                        <span class="text-xs sm:text-sm text-slate-800 font-bold font-mono">{{ info.laravel_version }}</span>
                                    </div>
                                    <div class="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-100 pb-2 gap-1 sm:gap-2">
                                        <span class="text-xs sm:text-sm text-slate-500 font-medium">Versión PHP:</span>
                                        <span class="text-xs sm:text-sm text-slate-800 font-bold font-mono">{{ info.php_version }}</span>
                                    </div>
                                    <div class="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-100 pb-2 gap-1 sm:gap-2">
                                        <span class="text-xs sm:text-sm text-slate-500 font-medium">Entorno (APP_ENV):</span>
                                        <span class="text-xs sm:text-sm text-slate-800 font-bold capitalize">{{ info.environment }}</span>
                                    </div>
                                    <div class="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-100 pb-2 gap-1 sm:gap-2">
                                        <span class="text-xs sm:text-sm text-slate-500 font-medium">Modo Debug:</span>
                                        <span class="text-xs sm:text-sm font-bold" :class="info.debug_mode.includes('Activado') ? 'text-amber-600' : 'text-slate-800'">{{ info.debug_mode }}</span>
                                    </div>
                                    <div class="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-100 pb-2 gap-1 sm:gap-2">
                                        <span class="text-xs sm:text-sm text-slate-500 font-medium">Carpeta Pública Activa:</span>
                                        <span class="text-xs sm:text-sm text-slate-800 font-bold font-mono truncate max-w-full" :title="info.public_dir">{{ info.public_dir }}</span>
                                    </div>
                                </div>
