...
const subscribeToPush = async () => {
    ...
    try {
        ...
        const vapidPublicKey = usePage().props.vapidPublicKey || 'BIreKgF_4XxbavEskkMWHsp65fBN233Wb0yfinIghrgrXbk6YOpMmnBPBMg29wVyL0J-SGj0GOSuX05abQE-OwA';
...
const probarPush = async () => {
    try {
        if ('serviceWorker' in navigator) {
            ...
            registration.showNotification('🧪 Prueba Push - Suraki Logística', {
                body: '¡Excelente! Las notificaciones de escritorio y móviles están activas y funcionando correctamente.',
                icon: '/icon.png',
                badge: '/badge.png',
...
const inactivitySecondsLeft = ref(60);
let inactivityTimer = null;
let countdownTimer = null;
const INACTIVITY_LIMIT_MS = 14 * 60 * 1000; // 14 minutos

const resetInactivityTimer = () => {
    if (usePage().props.auth.user.role === 'receptor') return;
    ...
    inactivityTimer = setTimeout(() => {
        ...
        inactivitySecondsLeft.value = 60;
...
// === BLOQUEO DE NAVEGACIÓN ATRÁS (Escritorio + Móvil) ===
const onPopState = (event) => {
    // Detenemos que Inertia u otros scripts procesen el "atrás"
...
onMounted(() => {
    ...
    // Bloquear botón atrás y gesto swipe-back en móvil de forma agresiva
    history.pushState(null, '', window.location.href);
...
// === CONTROL DE ACCESO A MÓDULOS (Permisos Granulares) ===
const tieneModulo = (modulo) => {
    const user = usePage().props.auth.user;
    if (!user) return false;
    // Superadmin Jeralth tiene acceso total a todos los módulos
    if (user.id === 1 || user.username === 'Sistemas.Jeralthc') return true;

    // Si tiene modulos_permitidos configurado como array no vacío, aplicar filtro
    if (Array.isArray(user.modulos_permitidos) && user.modulos_permitidos.length > 0) {
        return user.modulos_permitidos.includes(modulo);
    }

    // Fallback por defecto según rol
    if (user.role === 'admin') return true;
    if (user.role === 'receptor') return ['recepcion', 'monitor_odc', 'operarios', 'reservar_cita'].includes(modulo);
    if (user.role === 'comprador') return ['recepcion', 'monitor_odc', 'reservar_cita', 'monitoreo'].includes(modulo);
    if (user.role === 'proveedor') return ['recepcion', 'reservar_cita'].includes(modulo);
    return false;
};
</script>
...
<template>
    <div>
            ...
            <nav class="border-b border-gray-100 bg-white">
                    ...
                    <div class="flex h-16 justify-between">
                        <div class="flex">
                            ...
                            <div class="flex shrink-0 items-center">
                                <Link :href="$page.props.auth.user?.role === 'comprador' ? route('monitor-odc') : route('dashboard')">
                                    <ApplicationLogo
                            ...
                            <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                                ...
                                <NavLink
                                    v-if="tieneModulo('reservar_cita')"
                                ...
                                <NavLink
                                    v-if="tieneModulo('monitoreo')"
                                    :href="route('monitoreo')"
                                    :active="route().current('monitoreo') || route().current('auditoria')"
                                >
                        ...
                        <div class="flex items-center gap-2 sm:gap-4 sm:ms-6">
                            ...
                            <div class="relative" v-if="$page.props.auth.user.role === 'admin' || $page.props.auth.user.role === 'receptor' || $page.props.auth.user.role === 'proveedor'">
                                ...
                                >
                                    <div
                                        ...
                                        class="notif-panel fixed inset-x-3 top-16 sm:absolute sm:inset-auto sm:right-0 sm:top-full sm:mt-3 w-auto sm:w-[400px] max-w-[calc(100vw-1.5rem)] bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden z-50"
                                    >
                                        <!-- Header del panel -->
                                        <div class="bg-slate-900 px-4 py-3 sm:px-5 sm:py-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0">
                                            <div>
                                            ...
                                            <div class="flex items-center gap-2 sm:gap-3 flex-wrap">
                                                <button
                                                    ...
                                                    class="text-[10px] sm:text-xs px-2 py-1 sm:px-2.5 rounded-md bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 hover:bg-emerald-500/40 transition-colors font-semibold flex items-center gap-1.5"
                                                    title="Activar Notificaciones de Escritorio/Móvil"
                                                >
                                                ...
                                                <button
                                                    v-if="pushSupported && pushEnabled"
                                                    @click="probarPush"
                                                    class="text-[10px] sm:text-xs px-2 py-1 sm:px-2.5 rounded-md bg-blue-500/20 text-blue-400 border border-blue-500/30 hover:bg-blue-500/40 transition-colors font-semibold flex items-center gap-1.5"
                                                    title="Enviar una Notificación Push de Prueba"
                                                >
                                                    <span class="sm:hidden">🧪 Probar</span>
                                                    <span class="hidden sm:inline">🧪 Probar Push</span>
                                                </button>
                                                <button
                                                    v-if="totalNoLeidas > 0"
                                                    @click="marcarTodasLeidas"
                                                    class="text-[10px] sm:text-xs text-red-400 hover:text-red-300 font-bold transition-colors"
                                                >
                                                    Leer todas
                                                </button>
                                            </div>
                            ...
                            <!-- Settings Dropdown -->
                            <div class="hidden sm:block relative ms-3">
                ...
                >
                    <div class="space-y-1 pb-3 pt-2">
                        ...
                        <ResponsiveNavLink 
                            v-if="tieneModulo('monitoreo')"
                            :href="route('monitoreo')" :active="route().current('monitoreo') || route().current('auditoria')">
                            {{ $page.props.auth.user.role === 'comprador' ? '📡 Monitor & Trazabilidad' : 'Monitoreo & Auditoría' }}
                        </ResponsiveNavLink>
                        <ResponsiveNavLink 
                            v-if="tieneModulo('configuracion_erp')"
        ...
        <div v-if="showInactivityWarning" class="fixed inset-0 z-[110] flex items-center justify-center p-4">
            <div class="absolute inset-0 bg-slate-900/60 backdrop-blur-sm"></div>
            <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden animate-slide-up">
                <div class="bg-amber-500 px-6 py-4 flex items-center gap-3">
                    <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path></svg>
                    <h3 class="text-white font-bold text-lg">Inactividad Detectada</h3>
                </div>
                <div class="p-6 text-center">
                    <p class="text-sm text-slate-600 mb-4">Tu sesión está a punto de cerrarse debido a inactividad por seguridad.</p>
                    
                    <div class="text-4xl font-black text-amber-600 mb-6 flex justify-center items-end gap-1">
                        {{ inactivitySecondsLeft }}
                        <span class="text-lg font-bold text-amber-500 mb-1">segundos</span>
                    </div>

                    <button @click="stayOnline" class="w-full bg-amber-500 hover:bg-amber-600 text-white font-bold py-3 rounded-xl shadow-lg transition-colors text-sm">
                        Seguir Conectado
                    </button>
                </div>
            </div>
